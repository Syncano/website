...
{% highlight javascript linenos %}def ngram(sentence,n):
...
{% highlight javascript linenos %}#basic example
def thing(*x): print x
...
{% highlight javascript linenos %}def similarity_analysis(doc_one,doc_two):
...
{% highlight javascript linenos %}[ ("Hello there, I'm Eric","greeting"),
...
{% highlight javascript linenos %}"Hello there, I'm Eric" -> Freq(Hello) = 1,  Freq(there,) = 1,  Freq(I'm) = 1, Freq(Eric) = 1 ->
...
{% highlight javascript linenos %}cl = NaiveBayesClassifier(train)
...
{% highlight javascript linenos %}from sklearn.feature_extraction.text import TfidfVectorizer
