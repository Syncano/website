...
{% highlight javascript lineanchors %}$ docker pull debian:squeeze
{% endhighlight %}

<p>To see what Docker images are available on your machine, write:</p>

{% highlight javascript lineanchors %}$ docker images
...
{% highlight javascript lineanchors %}$ docker run [OPTIONS] IMAGE[:TAG] [COMMAND] [ARG...]
{% endhighlight %}

<p>Use this to test the image with a simple <em>ls</em> command run on your Docker image:</p>

{% highlight javascript lineanchors %}$ docker run -t ubuntu:14.04 ls
...
{% highlight javascript lineanchors %}$ docker run -t ubuntu:14.04
{% endhighlight %}

<p>or</p>

{% highlight javascript lineanchors %}$ docker run -i -t ubuntu:14:04 /bin/bash
{% endhighlight %}
...
{% highlight javascript lineanchors %}$ docker search redis
{% endhighlight %}

<p>You'll notice the command prints almost eight houndred images!</p>

{% highlight javascript lineanchors %}$ docker search redis | wc -l
779
{% endhighlight %}
...
{% highlight javascript lineanchors %}$ mkdir /var/docker/redis
{% endhighlight %}

<p>You can mount by:</p>

{% highlight javascript lineanchors %}$ docker run -v /host_directory:/container_directory docker_image
...
{% highlight javascript lineanchors %}$ docker run -p $HOSTPORT:$CONTAINERPORT --name CONTAINER -t someimage
{% endhighlight %}

<p>For this example, it should look like:</p>

{% highlight javascript lineanchors %}$ docker run -d -v /var/docker/redis:/data -p 6379:6379 --name=redis dockerfile/redis
{% endhighlight %}
...
{% highlight javascript lineanchors %}$ docker ps -a
{% endhighlight %}
...
{% highlight javascript lineanchors %}$ docker inspect <docker_container_id or name>
{% endhighlight %}

<p>If you run a Redis container with the <em>--name</em> flag, you can inspect it with the below command:</p>

{% highlight javascript lineanchors %}$ docker inspect redis
{% endhighlight %}
...
{% highlight javascript lineanchors %}$ docker logs redis
{% endhighlight %}

<p>Which will let you see what container prints to STDIO and STDOUT, and</p>

{% highlight javascript lineanchors %}$ docker top redis
{% endhighlight %}
